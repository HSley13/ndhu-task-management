...
async function openLocalFileOnAndroid(
  uri: string,
  mimeType: string,
): Promise<void> {
...
function fileIconAndColor(mime: string): {
  icon: IconName;
  color: string;
  bg: string;
} {
    ...
    <>
      ...
      <Pressable style={styles.imageCard} onPress={() => setModalVisible(true)}>
        <Image
          ...
          style={styles.imageCardThumb}
          resizeMode="cover"
        />
        <View style={styles.imageCardOverlay}>
          <Text style={styles.imageCardName} numberOfLines={2}>
            {attachment.name}
          </Text>
          {attachment.size_bytes > 0 && (
            <Text style={styles.imageCardSize}>
...
function FileCard({ attachment, onDelete }: Props) {
  ...
  async function handleOpen() {
    ...
    if (Platform.OS === "android" && isLocal) {
      ...
      setOpening(true);
      try {
        await openLocalFileOnAndroid(attachment.uri, attachment.mime_type);
      } catch {
  ...
  return (
    <>
      ...
      />
      <Pressable
        style={[styles.fileCard, opening && { opacity: 0.6 }]}
        onPress={handleOpen}
      >
  ...
  return (
    <View style={styles.audioCard}>
      <View
        style={[
          styles.audioIconBox,
          loadError && { backgroundColor: colors.bg.elevated },
        ]}
      >
        <Feather
          name="headphones"
