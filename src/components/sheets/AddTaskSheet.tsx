...
export function AddTaskSheet({ sheetRef, initialDate }: AddTaskSheetProps) {
  ...
  const [pendingReminderOffsets, setPendingReminderOffsets] = useState<number[]>([]);
      ...
      <BottomSheetScrollView contentContainerStyle={styles.content} keyboardShouldPersistTaps="handled">
        ...
        <View style={styles.descWrap}>
            ...
            <View style={styles.descAttList}>
              {pendingAttachments.map((att, i) => (
                <AttachmentRow
                  ...
                  onDelete={() => setPendingAttachments((p) => p.filter((_, idx) => idx !== i))}
                />
              ))}
            </View>
          )}
          ...
          <View style={styles.descAttRow}>
            <Pressable style={styles.descAttBtn} onPress={pickImage} hitSlop={6}>
            ...
            <Pressable
              style={[styles.descAttBtn, selectedLabels.length > 0 && styles.descAttBtnActive]}
            ...
            >
              <Feather
                name="tag"
                size={16}
                color={selectedLabels.length > 0 ? colors.accent.default : colors.text.secondary}
              />
            </Pressable>
            {selectedLabels.length > 0 && (
              <View style={styles.labelDotsRow}>
                {selectedLabels.slice(0, 4).map((id) => {
                  ...
                  return col ? <View key={id} style={[styles.labelDotSmall, { backgroundColor: col }]} /> : null;
                })}
              </View>
            )}
            {pendingAttachments.length > 0 && (
              <Text style={styles.descAttCount}>{pendingAttachments.length} attached</Text>
            )}
          </View>
        </View>
        ...
        <View style={styles.dateRow}>
          <Pressable style={styles.datePill} onPress={() => setShowDatePicker(true)}>
            <Feather name="calendar" size={14} color={dueDate ? colors.accent.default : colors.text.tertiary} />
          ...
          {dueDate && (
            <Pressable style={styles.datePill} onPress={() => setShowTimePicker(true)}>
              <Feather name="clock" size={14} color={dueTime ? colors.accent.default : colors.text.tertiary} />
          ...
          {dueDate && (
            ...
            >
              <Feather
                name="bell"
                size={14}
                color={pendingReminderOffsets.length > 0 ? colors.accent.default : colors.text.tertiary}
              />
              <Text style={[styles.datePillText, pendingReminderOffsets.length > 0 && styles.dateActive]}>
        ...
        {showDatePicker && Platform.OS !== 'web' && (
          <NativeDatePicker
            ...
            onChange={(_: any, d?: Date) => { setShowDatePicker(false); if (d) setDueDate(d); }}
        ...
        {showDatePicker && Platform.OS === 'web' && (
          <TextInput
            // @ts-ignore - React Native Web passes type to underlying <input>
            type="date"
            value={dueDate ? format(dueDate, 'yyyy-MM-dd') : ''}
            onChangeText={(v) => { setShowDatePicker(false); if (v) setDueDate(parseISO(v)); }}
        ...
        {showTimePicker && Platform.OS !== 'web' && (
          <NativeDatePicker
            ...
            mode="time"
            display={Platform.OS === 'ios' ? 'spinner' : 'default'}
            onChange={(_: any, d?: Date) => { setShowTimePicker(false); if (d) setDueTime(d); }}
            textColor="#FFFFFF"
          />
        )}
        {showTimePicker && Platform.OS === 'web' && (
        ...
        {dueDate && (
          <ReminderPickerModal
            ...
            onRemoveCustom={(offset) =>
              setPendingReminderOffsets((prev) => prev.filter((o) => o !== offset))
            }
          />
        )}
