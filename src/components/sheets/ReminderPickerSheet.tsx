...
export function ReminderPickerSheet({ sheetRef, taskId }: ReminderPickerSheetProps) {
      ...
      <View style={styles.content}>
        ...
        <View style={styles.list}>
          {OFFSET_OPTIONS.map((opt) => (
            <Pressable key={opt.value} style={styles.row} onPress={() => toggle(opt.value)}>
              <Text style={styles.label}>{opt.label}</Text>
              <View style={[styles.check, selected.includes(opt.value) && styles.checkActive]}>
        ...
        <Button label="Save Reminders" onPress={handleSave} loading={loading} disabled={selected.length === 0} />
      </View>
    </BottomSheet>
  );
}
